-- =====================================================
-- Script SQL pour insérer toutes les données
...
DECLARE
    ...
    zone_id INTEGER;
BEGIN
    SELECT id INTO zone_centre_id FROM zone WHERE zone.code = 'ZONE-001';
    SELECT id INTO zone_akanda_id FROM zone WHERE zone.code = 'ZONE-002';
    SELECT id INTO zone_ntoum_id FROM zone WHERE zone.code = 'ZONE-003';
    SELECT id INTO zone_owendo_id FROM zone WHERE zone.code = 'ZONE-004';
...
('Artisan', 'TC-009', 'Artisan', true),
('Prestataire', 'TC-010', 'Prestataire', true),
('Vendeur ambulant', 'TC-011', 'Vendeur ambulant', true),
('Taxi', 'TC-012', 'Taxi', true),
('Moto-taxi', 'TC-013', 'Moto-taxi', true),
('Garage', 'TC-014', 'Garage', true),
('Pharmacie', 'TC-015', 'Pharmacie', true),
('Superette', 'TC-016', 'Superette', true),
('Boulangerie', 'TC-017', 'Boulangerie', true),
('Coiffure', 'TC-018', 'Coiffure', true),
('Salon de beauté', 'TC-019', 'Salon de beauté', true),
('Cybercafé', 'TC-020', 'Cybercafé', true),
('Imprimerie', 'TC-021', 'Imprimerie', true),
('Photographe', 'TC-022', 'Photographe', true),
('Bijoutier', 'TC-023', 'Bijoutier', true)
ON CONFLICT (code) DO NOTHING;
...
('Service de la Propreté', 'SRV-008', 'Service de la Propreté', true),
('Service de l''Éclairage Public', 'SRV-009', 'Service de l''Éclairage Public', true),
('Service des Espaces Verts', 'SRV-010', 'Service des Espaces Verts', true),
('Service de la Sécurité', 'SRV-011', 'Service de la Sécurité', true),
('Service de la Communication', 'SRV-012', 'Service de la Communication', true)
ON CONFLICT (code) DO NOTHING;
...
BEGIN
    FOR i IN 1..50 LOOP
        ...
        matricule_val := 'COL-' || LPAD(i::TEXT, 3, '0');
...
BEGIN
    FOR i IN 1..50 LOOP
        nom := noms[1 + (i % array_length(noms, 1))];
        prenom := prenoms[1 + (i % array_length(prenoms, 1))];
        telephone_val := '+24106' || LPAD((2000000 + i)::TEXT, 7, '0');
        ...
        -- Vérifier si le contribuable existe déjà avant insertion
        IF NOT EXISTS (
            SELECT 1 FROM contribuable c
            WHERE c.telephone = telephone_val 
            OR c.numero_identification = numero_id
        ) THEN
...
BEGIN
    -- Taxes de base
    SELECT id INTO type_taxe_id FROM type_taxe WHERE type_taxe.code = 'TT-001'::TEXT LIMIT 1;
    SELECT id INTO service_id FROM service WHERE service.code = 'SRV-002'::TEXT LIMIT 1;
    ...
    FOR i IN 3..50 LOOP
        SELECT tt.id INTO type_taxe_id FROM type_taxe tt ORDER BY RANDOM() LIMIT 1;
        SELECT s.id INTO service_id FROM service s ORDER BY RANDOM() LIMIT 1;
        montant := montants[1 + (i % array_length(montants, 1))];
        periodicite := periodicites[1 + (i % array_length(periodicites, 1))];
        code_taxe := 'TAX-' || LPAD(i::TEXT, 3, '0');
...
DECLARE
    i INTEGER;
    contribuable_id_val INTEGER;
    taxe_id_val INTEGER;
    date_debut_val TIMESTAMP;
    affectation_count INTEGER := 0;
BEGIN
    FOR contribuable_id_val IN SELECT id FROM contribuable LOOP
        -- Chaque contribuable a 1-3 taxes
        FOR i IN 1..(1 + (contribuable_id_val % 3)) LOOP
            SELECT id INTO taxe_id_val FROM taxe ORDER BY RANDOM() LIMIT 1;
            date_debut_val := CURRENT_TIMESTAMP - (RANDOM() * 180 || ' days')::INTERVAL;
            ...
            VALUES (
                ...
                taxe_id_val,
                date_debut_val,
                CASE WHEN (contribuable_id_val % 4) = 0 THEN date_debut_val + INTERVAL '365 days' ELSE NULL END,
                CASE WHEN (contribuable_id_val % 3) = 0 THEN (SELECT montant FROM taxe WHERE taxe.id = taxe_id_val) * (0.8 + RANDOM() * 0.7) ELSE NULL END,
                true
            )
...
DECLARE
    ...
    contribuable_id_val INTEGER;
    taxe_id_val INTEGER;
    collecteur_id_val INTEGER;
    affectation_id_val INTEGER;
    montant_val NUMERIC;
    commission_val NUMERIC;
    type_paiement_val TEXT;
    statut_val TEXT;
    reference_val TEXT;
    date_collecte_val TIMESTAMP;
    billetage_val TEXT;
BEGIN
    FOR i IN 1..50 LOOP
        SELECT at.id, at.contribuable_id, at.taxe_id INTO affectation_id_val, contribuable_id_val, taxe_id_val
        FROM affectation_taxe at
        WHERE at.actif = true
        ORDER BY RANDOM()
        LIMIT 1;
        
        SELECT id INTO collecteur_id_val FROM collecteur ORDER BY RANDOM() LIMIT 1;
        SELECT montant INTO montant_val FROM taxe WHERE taxe.id = taxe_id_val;
        
        commission_val := montant_val * (SELECT commission_pourcentage FROM taxe WHERE taxe.id = taxe_id_val) / 100;
        type_paiement_val := (ARRAY['especes', 'mobile_money', 'carte'])[1 + (i % 3)];
        statut_val := (ARRAY['completed', 'completed', 'completed', 'pending', 'failed'])[1 + (i % 5)];
        date_collecte_val := CURRENT_TIMESTAMP - (RANDOM() * 90 || ' days')::INTERVAL;
        reference_val := 'COL-' || TO_CHAR(date_collecte_val, 'YYYYMMDD') || '-' || LPAD(i::TEXT, 4, '0');
        
        -- Générer billetage pour espèces
        IF type_paiement_val = 'especes' AND (i % 2) = 0 THEN
            billetage_val := json_build_object(
                '5000', (i % 10),
                '1000', (i % 20)
            )::TEXT;
        ELSE
        ...
        VALUES (
            ...
            montant_val,
            commission_val,
            type_paiement_val::type_paiement_enum,
            statut_val::statut_collecte_enum,
            reference_val,
            billetage_val,
            date_collecte_val,
            CASE WHEN (i % 3) = 0 THEN date_collecte_val + (RANDOM() * 8 || ' hours')::INTERVAL ELSE NULL END,
            (i % 2) = 0,
            (i % 2) = 0,
            false
        )
...
DECLARE
    ...
    telephone TEXT;
    role_val TEXT;
    password_hash TEXT := '$2b$12$EzmNLFv/pObUcST4WrLDTeff5edcwmbtgc8nzr1fLWJlpzM6nCHum'; -- hash de "password123"
BEGIN
    -- Admin
    ...
    VALUES (
        ...
        '+241062345678',
        '$2b$12$/ajO5nZAc4NnV/VxGwPx5eql919P.SU4VAX9pA165.MZ6jAxxZqr2', -- hash de "admin123"
        'admin'::role_enum,
        true
    )
    ...
    FOR i IN 1..49 LOOP
        nom := noms[1 + (i % array_length(noms, 1))];
        prenom := prenoms[1 + (i % array_length(prenoms, 1))];
        email_val := 'user' || i || '@mairie-libreville.ga';
        telephone := '+24106' || LPAD((3000000 + i)::TEXT, 7, '0');
        role_val := roles[1 + (i % array_length(roles, 1))];
