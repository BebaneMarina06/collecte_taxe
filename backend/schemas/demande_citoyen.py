...
Remplacez l'import et la déclaration du router
"""
...
from auth.security import get_current_user_or_citoyen  # ← CHANGÉ ICI

router = APIRouter(
    prefix="/api/demandes",
    tags=["demandes-citoyens"],
    dependencies=[Depends(get_current_user_or_citoyen)],  # ← CHANGÉ ICI
)
...
def update_demande(
    ...
    current_user = Depends(get_current_user_or_citoyen),  # ← CHANGÉ ICI
    db: Session = Depends(get_db)
):
